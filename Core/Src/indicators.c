...
// Note: structure indicator_output_pattern_t and enum indicator_led_t are diefined in the header file indicators.h

/* ===================  This is the sequence of functions calls that happen to control LED and buzzer  ===================

Firmware event like test_success generates the following sequence:
...
// LED selected for the currently active pattern. Only one LED can be controlled at a time.
static indicator_led_t active_led = INDICATOR_LED_NONE;

// Independent timing configurations for the selected LED and the buzzer.
...
The LED and buzzer use this same reference time, 
but each applies its own start_delay_ms.
*/
...
LEDs and the buzzer are active high.
*/
...
This assumes an active high  buzzer.
*/
...
*/
static bool indicator_output_pattern_is_valid( const indicator_output_pattern_t *pattern)
{
    ...
    // An enabled output requires a nonzero ON and non-zero total duration.
    if (pattern->on_duration_ms == 0U  || pattern->total_duration_ms == 0U )
    {
        return false;
    }
...
*/
static bool indicator_calculate_output_state( uint32_t elapsed_pattern_ms, const indicator_output_pattern_t *pattern, bool *output_finished)
{
    ...
    output_elapsed_ms = elapsed_pattern_ms - pattern->start_delay_ms;
    ...
    */
    ...
    cycle_duration_ms = pattern->on_duration_ms + pattern->off_duration_ms;

    // Calculate the current ms position inside the cycle.
    cycle_position_ms = output_elapsed_ms % cycle_duration_ms;
    ...
    // When the previous logc is not met, the output is in its OFF phase.
    return false;
}
...
The timing configurations are copied into static variables.
The caller's structures are not required after this function returns.
*/
bool indicator_start_pattern( indicator_led_t led, const indicator_output_pattern_t *led_pattern, const indicator_output_pattern_t *beep_pattern)
{
    // Both configuration pointers must be valid.
    if ((led_pattern == NULL) || (beep_pattern == NULL))
    {
        return false;
    }

    // An enabled LED pattern needs an actual selected LED.
    if (led_pattern->enabled && (led == INDICATOR_LED_NONE))
    {
        return false;
    }
    ...
    if (!led_pattern->enabled && !beep_pattern->enabled)
    {
        return false;
    }
    ...
    indicator_process applies the correct output states on the next main-loop iteration.
    */
...
void indicator_process(void)
{
    ...
    */
    elapsed_pattern_ms = HAL_GetTick() - pattern_start_time_ms;

    // Calculate the current LED state.
    led_output_on = indicator_calculate_output_state( elapsed_pattern_ms, &active_led_pattern, &led_finished);

    // Calculate the current buzzer state.
    beep_output_on = indicator_calculate_output_state( elapsed_pattern_ms, &active_beep_pattern,  &beep_finished);
...
// check whether an indicator pattern is currently active.
...
// Place this inside the private variable section of main.c

/*

// Green LED starts after two short beeps and stays on for 3 seconds.
static const indicator_output_pattern_t success_led_pattern =
{
    .enabled = true,
    .start_delay_ms = 300U,
    .on_duration_ms = 3000U,
    .off_duration_ms = 0U,
    .total_duration_ms = 3000U
};


// Two short beeps before the green LED starts.
static const indicator_output_pattern_t success_beep_pattern =
{
    .enabled = true,
    .start_delay_ms = 0U,
    .on_duration_ms = 100U,
    .off_duration_ms = 100U,
    .total_duration_ms = 300U
};


// Red LED blinks for 3 seconds.
static const indicator_output_pattern_t error_led_pattern =
{
    .enabled = true,
    .start_delay_ms = 0U,
    .on_duration_ms = 300U,
    .off_duration_ms = 300U,
    .total_duration_ms = 3000U
};


// Buzzer beeps independently during an error.
static const indicator_output_pattern_t error_beep_pattern =
{
    .enabled = true,
    .start_delay_ms = 0U,
    .on_duration_ms = 500U,
    .off_duration_ms = 500U,
    .total_duration_ms = 3000U
};


// No LED output.
static const indicator_output_pattern_t led_disabled_pattern =
{
    .enabled = false,
    .start_delay_ms = 0U,
    .on_duration_ms = 0U,
...
// One short beep.
static const indicator_output_pattern_t short_beep_pattern =
{
    .enabled = true,
    .start_delay_ms = 0U,
    .on_duration_ms = 80U,
    .off_duration_ms = 0U,
    .total_duration_ms = 80U
};


static bool test_just_passed = false;
static bool test_just_failed = false;
static bool encoder_just_changed = false;

*/


// Example to call the function from main.c
/*

while (1)
{
    
    //Run the main cable tester logic.

    //This function can set event flags such as:
        //test_just_passed
        //test_just_failed
        //encoder_just_changed
    
    cable_tester_process();


    // Start the success indication only once when the test passes.
    if (test_just_passed)
    {
        indicator_start_pattern(
            INDICATOR_LED_GREEN,
            &success_led_pattern,
            &success_beep_pattern);

        // Clear the event so the pattern is not restarted.
        test_just_passed = false;
    }


    // Start the error indication only once when the test fails.
    if (test_just_failed)
    {
        indicator_start_pattern(
            INDICATOR_LED_RED,
        ...
        // Clear the event so the pattern is not restarted.
        test_just_failed = false;
    }


    // Produce one short beep when the encoder position changes.
    if (encoder_just_changed)
    {
        indicator_start_pattern(
            INDICATOR_LED_NONE,
            &led_disabled_pattern,
            &short_beep_pattern);

        // Clear the event so the beep is not restarted.
        encoder_just_changed = false;
    }


    
    //Update the LED and buzzer outputs.

    //This function must be called continuously.
    //It does not block and does not restart the pattern.
